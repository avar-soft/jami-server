...
echo -e "  DHT прокси : ${GREEN}${DHT_PROXY_PORT}${NC}"
echo ""
ask "Порт DHT прокси-сервера" DHT_PROXY_PORT "8080"
...
cat > /etc/systemd/system/dhtnode-proxy.service <<EOF2
[Unit]
Description=OpenDHT Proxy Server for Jami
After=network-online.target dhtnode.service
Wants=network-online.target
...
Group=dhtnode
WorkingDirectory=/var/lib/dhtnode
ExecStart=${DHTNODE_BIN} -s -p ${DHT_PORT} -b bootstrap.jami.net:4222 --proxyserver ${DHT_PROXY_PORT}
Restart=on-failure
RestartSec=10
LimitNOFILE=65536
StandardInput=null
...
success "DHT прокси-сервис настроен (порт $DHT_PROXY_PORT/TCP)"
...
ufw allow ${DHT_PROXY_PORT}/tcp        comment "DHT proxy"         >/dev/null
...
systemctl enable dhtnode-proxy >/dev/null 2>&1
systemctl restart dhtnode-proxy
sleep 2
if systemctl is-active --quiet dhtnode-proxy; then
    success "dhtnode-proxy запущен"
else
    warn "dhtnode-proxy не запустился — проверьте: journalctl -u dhtnode-proxy -n 30"
fi
...
check_port $DHT_PROXY_PORT tcp "DHT proxy"
...
--- DHT прокси ---
URI:       http://${SERVER_IP}:${DHT_PROXY_PORT}
